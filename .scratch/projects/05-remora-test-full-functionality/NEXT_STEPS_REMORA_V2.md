...
Date: 2026-03-24  
Target repository: `/home/andrew/Documents/Projects/remora-v2`
...
5. fix subscription path filtering so `path_glob` works reliably for `node_changed`/`node_discovered` triggers.
6. ensure companion can activate in normal flows by emitting/propagating `turn_digested` where intended.
...
3. enabling `path_glob` on review subscriptions does not suppress valid source-change triggers.
4. companion observer receives actionable turn-digest signals in default runtime flow.
